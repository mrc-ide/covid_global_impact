...
CVD_age <- final_df %>%
  ...
  mutate(infected_with_comorb_per_1000 = 1000 * total_infected_with_comorb/total_pop) %>%
...
HIC_averted/averted
sum(CVD_age$total_infected_with_comorb)/sum(final_df$number_infected[final_df$Strategy == "Unmitigated" & final_df$cause_name == "Cardiovascular diseases"])

a <- ggplot(CVD_age, aes(x = income_group, y = infected_with_comorb_per_1000, fill = age_group)) +
  geom_bar(stat = "identity") + 
  facet_grid(~Strategy) + 
  theme_bw() +
  labs(y = "Infections Per 1000", x = "") +
...
c <- ggplot(hiv_age, aes(x = income_group, y = infected_with_comorb_per_1000, fill = age_group)) +
  geom_bar(stat = "identity") + 
  facet_grid(~Strategy) + 
  theme_bw() +
  labs(y = "Infections Per 1000", x = "") +
...
e <- ggplot(nutrition_age, aes(x = income_group, y = infected_with_comorb_per_1000, fill = age_group)) +
  geom_bar(stat = "identity") + 
  facet_grid(~Strategy) + 
  theme_bw() +
  labs(y = "Infections Per 1000", x = "") +
...
sum(COPD_age$total_infected_with_comorb)/sum(final_df$number_infected[final_df$Strategy == "Unmitigated" & final_df$cause_name == "Cardiovascular diseases"])


g <- ggplot(COPD_age, aes(x = income_group, y = infected_with_comorb_per_1000, fill = age_group)) +
  geom_bar(stat = "identity") + 
  facet_grid(~Strategy) + 
  theme_bw() +
  labs(y = "Infections Per 1000", x = "") +
...
COPD <- final_df %>%
  ...
  group_by(income_group, age_group) %>%
  summarise(prop_comorb = sum(number_comorb)/sum(age_population))
h <- ggplot(COPD, aes(income_group, age_group, fill = prop_comorb)) + 
  geom_tile() +
  labs(y = "", x = "") +
  scale_fill_distiller(palette = "OrRd", direction = 1,
...
sum(diabetes_age$total_infected_with_comorb)/sum(final_df$number_infected[final_df$Strategy == "Unmitigated" & final_df$cause_name == "Cardiovascular diseases"])


i <- ggplot(diabetes_age, aes(x = income_group, y = infected_with_comorb_per_1000, fill = age_group)) +
  geom_bar(stat = "identity") + 
  facet_grid(~Strategy) + 
  theme_bw() +
  labs(y = "Infections Per 1000", x = "Income Group") +
  scale_fill_manual(breaks = levels(COPD_age$age_group),
                    values = rev(magma(length(levels(COPD_age$age_group))))) +
  theme(legend.position = "none",
        strip.background = element_blank(),
...
j <- ggplot(diabetes, aes(income_group, age_group, fill = prop_comorb)) + 
  geom_tile() +
  labs(y = "Infections Per 1000", x = "") +
  scale_fill_distiller(palette = "OrRd", direction = 1,
...
k <- ggplot(tb_age, aes(x = income_group, y = infected_with_comorb_per_1000, fill = age_group)) +
  geom_bar(stat = "identity") + 
  facet_grid(~Strategy) + 
  theme_bw() +
  labs(y = "Infections Per 1000", x = "Income Group") +
...
l <- ggplot(tb, aes(income_group, age_group, fill = prop_comorb)) + 
  geom_tile() +
  labs(y = "Infections Per 1000", x = "") +
  scale_fill_distiller(palette = "OrRd", direction = 1,
...
layout <- "AABBBBBBCCDDDDDD
EEFFFFFFGGHHHHHH
IIJJJJJJKKLLLLLL"

figure <- b + a + d + c + h + g + f + e + j + i + l + k +
  plot_layout(design = layout)
figure 
pdf("Figure_3_Comorbidities_and_Infections.pdf", width = 13, height = 10)
figure # save this manually width = 13, height = 10
dev.off()