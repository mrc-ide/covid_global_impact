# Get Total Time Spent In Lockdown
get_time_in_lockdown <- function(trigger_output) {
  ...
  return(overall_time_in_lockdown/length_output)
}

# Get Maximum ICU Requirements
get_max_ICU_req <- function(x) {
  ...
  for (i in 1:replicates) {
    indices <- c(index$IMVGetDie1, index$IMVGetDie2, index$IMVGetLive1, index$IMVGetLive2)
    ICU_occupancy <- apply(out[, indices, i], 1, sum)
    daily_ICU_occupancy <- rollapply(ICU_occupancy, 10, mean, partial = TRUE, align = "right")
    daily_ICU_occupancy <- daily_ICU_occupancy[seq(1, length(daily_ICU_occupancy), 10)]
    max_ICU_occupancy[i] <- max(daily_ICU_occupancy)
  }
  return(max_ICU_occupancy)
}
...
realistic_run_trigger_threshold <- function(country, population, income_strata,
  ...
  # Setting Seed So That Initial Death Triggered Run Is Identical
  set.seed(1000019)
  ...
  for (i in 1:max_lockdowns) {
    ...
    if (i == 1) {
      ...
      trigger_times <- lapply(seq_along(trigger_times), function(x){
        req <-  apply(out[, index$number_requiring_IMV, x], 1, sum)
        daily_ICU_incidence <- rollapply(req, 1/dt, sum, partial = TRUE, align = "right")
...
process_output <- function(output, index, ICU_occupancy_indices, scenario) {
  time <- output$model_output[, index$time, 1]
  y <- lapply(seq_along(1:dim(output$model_output)[3]), function(p) {
    temp <- apply(output$model_output[, ICU_occupancy_indices, p], 1, sum)
  })
  y <- do.call(cbind, y)
...
# Generic Trigger Running Function
