...
merged_for_sim$deaths_at_end_exponential
...
merged_for_sim[merged_for_sim$country=="Sweden",]$deaths_at_end_exponential
### CALCULATE INFECTION DURING ASSUMED EXPONENTIAL PHASE
Rs=c(rep(c(2.7,3,3.5),each=100))
...
sum(cali$parameters$population)

summary(infections_output[2,])
hj?align
plot(1:300,infections_output[2,])

  
)
format_output(cali, var_select = "infections")$y
x
x$infections[1]
cali$parameters
3809/(merged_for_sim$deaths_at_end_exponential[2]*3200)
summary(merged_for_sim$cases_at_end_exponential[2]/infections_output[2,])
summary(infections_output[2,])
