...
suppressed_countries <- data %>%
  ...
  mutate(Code=countryterritoryCode,end_exp_growth=Date_suppression)%>%
  group_by(Code,Region,Income_group,WPP_Population,Date_suppression,end_exp_growth)%>%
  ...
  ungroup() %>%
  select(Code,Region,Income_group,WPP_Population,Date_suppression,end_exp_growth,cases_at_suppression,deaths_at_suppression,cases_at_end_exponential,deaths_at_end_exponential)
...
merge_suppression<-merge(ACAPS_inputs,suppressed_countries,by=c("Code"),all.x = TRUE)  
...
ALL_expo_growth<-rbind(suppressed_countries,no_suppression_to_date_countries,undetermined_countries)%>%
  ungroup()%>%
  mutate("Code"=countryterritoryCode)%>%
  select(Code,end_exp_growth,deaths_at_end_exponential,cases_at_end_exponential)

### GET ALL COUNTRIES WITH >=3 DEATHS DURING ASSUMED EXPONENTIAL PHASE
cases_per_death<-tbl_df(rbind(suppressed_countries,no_suppression_to_date_countries,undetermined_countries))%>%
  filter(deaths_at_end_exponential>2)%>%
  mutate("Code"=countryterritoryCode)%>%
  group_by(Code)%>%
  summarise(cases_per_death=cases_at_end_exponential/deaths_at_end_exponential)

ALL_expo_growth<-merge(ALL_expo_growth,cases_per_death,all.x = TRUE)

### MERGE EVERYTHING AND OUTPUT
ACAPS_with_ECDC<-merge(ACAPS_inputs,ALL_expo_growth,by=c("Code"),all.x = TRUE)     
...
write.csv(ACAPS_with_ECDC,"Outputs_for_ACAPS_appendix_check.csv",row.names = F)
