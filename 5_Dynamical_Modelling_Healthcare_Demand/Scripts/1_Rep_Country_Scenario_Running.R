...
for (i in 1:length(time_for_triggers)) {
  temp_incidence_storage <- matrix(nrow = time_period_new, ncol = replicates)
  temp_hospitalisation_storage <- matrix(nrow = time_period_new, ncol = replicates)
  temp_ICU_storage <- matrix(nrow = time_period_new, ncol = replicates)
...
lifted_incidence_suppression_output <- data.frame(country_indicator = rep(country_indicator, time_period_new * 3),
                                                  ...
                                                  rbind(lower_incidence_output, median_incidence_output, upper_incidence_output))
colnames(lifted_incidence_suppression_output) <- c("time", "measure", "none", "incomplete")
