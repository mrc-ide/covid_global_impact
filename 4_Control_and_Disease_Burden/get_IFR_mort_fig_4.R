...
sensitivity_runs_var_select <- function(params) {
  ...
  to_keep <- c(index$t, index$time, index$n_E2_I, index$delta_D, index$D,index$IMVGetLive1,index$IMVGetLive2,index$IMVGetDie1,index$IMVGetDie2)
...
HIC_mit_FC_out_icu <- format_output(HIC_mit_FC, var_select = c("ICU_occupancy"), date_0 = Sys.Date())

HIC_mit_FC_out_icu <- format_output(HIC_mit_FC, var_select = c("ICU_occupancy"), date_0 = Sys.Date())


LMIC_mit_FC_out_icu <- format_output_new(LMIC_mit_FC, var_select = c("ICU_occupancy"), date_0 = Sys.Date())


LMIC_ICU<-data.frame("Income"="LMIC","demand"=sapply(1:ndraws,function(z){max(LMIC_mit_FC_out_icu$y[LMIC_mit_FC_out_icu$replicate==z])}),
